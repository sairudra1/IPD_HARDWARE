...
HardwareSerial mySerial(2); // Using UART2 for second ESP32
...
void setup() {
  ...
  mySerial.begin(115200, SERIAL_8N1, 16, 17); // UART on GPIO 16 (RX) and 17 (TX)
...
*/
    int nitrogen = random(65, 67);
    int pot = random(55, 56);
    int posp = random(24, 26);
    int pH = random(6, 7);
...
void processLine(String line) {
  if (line.startsWith("  ")) {
    ...
    int confStart = line.indexOf("(") + 1;
    int confEnd = line.indexOf(")");
    if (confStart > 0 && confEnd > confStart) {
      float confidence = line.substring(confStart, confEnd).toFloat();
      if (confidence > CONFIDENCE_THRESHOLD) {
        int nameEnd = line.indexOf(" (");
        if (nameEnd > 2) {
          String objectName = line.substring(2, nameEnd);

          Serial.println("main data ");
          Serial.println(objectName); // Print only high-confidence objects
          int insectt;
          if (objectName == "Butterfly") {
            insectt = 2;
            if (Firebase.RTDB.setInt(&fbdo, "sensor/Insect", insectt)) {
              Serial.print("object : ");
